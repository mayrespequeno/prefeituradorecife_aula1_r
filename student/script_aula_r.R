...
# Descrição: Esse é o script de linguagem R base do Workshop da Secretaria Executiva de Planejamento e Gestão (SEPLAN/SEPLAGTD)
...
install.packages("rio") # importar dados de qualquer classe
library(tidyverse)
library(here)
library(magrittr)
library(rio)
...
#Objetos, Classes e Funções ------------------------------------------------------------
 ...
 # Salvando 4 am a
 a <- 4 # atribuição, o R o sinal de = como atribuição
...
# o operador + verifica que a e b não são números (ou que a classe não é númerica)
...
vetor <- c(1, 2, 3) #anterior a C = c minusculo
...
idade_ana = idade_beto #atribuição
idade_ana <- idade_beto #atribuição
...
paste(nome, estado, sep = " - ") # colar essas duas strings seguidas de um sepador
...
mean(1:1000) # vá de tal elemento a tal elemento
...
#PAUSA
#DataFrames,Tibble, Operadores Lógicos e o Pipe------------------------------------------------------------
...
#Todos os elementos de um vetor devem ser do mesmo tipo de dado (numérico, caractere, lógico, etc.).
...
#Podem conter elementos de diferentes tipos de dados, incluindo outros vetores ou até mesmo outras listas.
...
#vamos estudar devagar as operações 
...
#Vamos usar o data frame equipe para manipular os dados usando o pipe
...
#Quantos Jovens entre 15 a 29 anos temos ao longo dos anos dos Censos em Recife?
#(O valor final deve ser apresentado por um único resultado no console)
